...
static void readTrim(i2c_port_t i2c_num)
{
    ...
    if(sensor_type==2)  //select BMP180
    {
        ...
        (i2c_m_read(i2c_num, BMP280_DIG_AC1_MSB_REG, &data[0], 22));
    ...
    else    //select BMP280 or BME280
    {
        ...
        if (sensor_type == 1)
        {
            (i2c_m_read(i2c_num, BMP280_DIG_H1_LSB_REG, &data[24], 1));
            (i2c_m_read(i2c_num, BMP280_DIG_H2_LSB_REG, &data[25], 7));
            dig_H1 = data[24];
...
static esp_err_t bme280_bmp280_init(i2c_port_t i2c_num)
{
    ...
    {
        ESP_LOGE(TAG, "No chip id, Get fail sensor..%x...\n", rbuf);
    }
    ...
    // {
    //     (i2c_m_write(i2c_num,BME280_CTRLHUM_REG, &ctrl_hum_reg, 1));
    // }
...
static void readData(i2c_port_t i2c_num)
{
    ...
    if (sensor_type == 1 )
    {
        (i2c_m_read(I2C_EXAMPLE_MASTER_NUM, BMX280_PRESSURE_MSB_REG, &data[0], 8));
...
static uint16_t readUT(i2c_port_t i2c_num)
{
    ...
    buf[0] =0x2e;
    (i2c_m_write(i2c_num, BMX280_CTRLMEAS_REG, &buf[0], 1));

    vTaskDelay(10 / portTICK_RATE_MS);


    (i2c_m_read(i2c_num, BMP180_DATA_MSB_REG, buf, 2));
...
static int32_t readUP(i2c_port_t i2c_num)
{
    ...
    (i2c_m_read(i2c_num, BMP180_DATA_MSB_REG, buf, 3));
