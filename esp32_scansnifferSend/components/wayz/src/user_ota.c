...
bool _esp_is_ota_updata(const char *text)
{
    char *ptr = NULL, *ptr2 = NULL;
    char length_str[32];
    char nvs_str[32];
    ptr = (char *)strstr(text, "Last-Modified");
    if (NULL == ptr)
    {
        return false;
    }
    ptr += 15;
    ptr2 = (char *)strstr(ptr, "GMT");
    memcpy(length_str, ptr, ptr2 - ptr);
    NVS_Read(AP_INFO_KEY, nvs_str, sizeof (nvs_str));
    printf("length_str: %s, nvs_str: %s \r\n", length_str, nvs_str);
    if (0 == memcmp(nvs_str, length_str, strlen(length_str)))
    {
        return false;
    }
    NVS_Write(AP_INFO_KEY, length_str, strlen(length_str));
    return true;
}
...
bool _esp_ota_firm_parse_http(esp_ota_firm_t *ota_firm, const char *text, size_t total_len, size_t *parse_len)
{
    ...
    while (text[i] != 0 && i < total_len) {
        if (ota_firm->content_len == 0 && (ptr = (char *)strstr(text, "Content-Length")) != NULL) {
            ...
            if (!_esp_is_ota_updata(text))
            {
                printf("no need ota updata\r\n");
                return false;
            }
...
static void ota_example_task(void *pvParameter)
{
    ...
    const char *GET_FORMAT =
        "GET /ota_file/%s HTTP/1.0\r\n"
        "Host: %s:%s\r\n"
        "User-Agent: esp-idf/1.0 esp32\r\n\r\n";
    vTaskSuspend(ota_handle);
while (1)
{
...
task_fatal_error:
    ...
    close(socket_id);
    xEventGroupSetBits(wifi_event_group, OTA_BIT);
    vTaskSuspend(ota_handle);
}
    return ;
}
...
void ota_resume()
{
    vTaskResume(ota_handle);
}

void create_ota_updata()
{
    xTaskCreate(ota_example_task, "ota_example_task", 4096, NULL, 11, &ota_handle);
}
